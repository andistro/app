...
apt update
apt install --option=Dpkg::Options::="--force-confold" bash -y
apt install --option=Dpkg::Options::="--force-confold" openssl -y
apt upgrade -y
apt install curl wget dialog tar unzip xz-utils dbus debootstrap proot termux-exec -y
...
sudo apt install dconf-cli lsb-release exo-utils tigervnc-standalone-server tigervnc-common tigervnc-tools sterm xorg dbus-x11 nautilus font-manager synaptic gvfs-backends --no-install-recommends -y
